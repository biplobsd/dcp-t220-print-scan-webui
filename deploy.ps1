...
# 1. Build the Docker image locally for ARM64 (Raspberry Pi architecture)
Write-Host "--> [1/4] Building Docker image for linux/arm64..." -ForegroundColor Blue
docker buildx build --platform linux/arm64 -t print-scan-webui:latest --load .

# 2. Save the image to a tarball
Write-Host "--> [2/4] Saving Docker image to print-scan-webui.tar..." -ForegroundColor Blue
...
# 3. Transfer the image to the Raspberry Pi
Write-Host "--> [3/4] Transferring image tarball to Raspberry Pi via scp..." -ForegroundColor Blue
scp print-scan-webui.tar pi@192.168.0.10:/home/pi/print-scan-webui.tar
Remove-Item "print-scan-webui.tar"

# 4. Stop native service, load image and start the container on the Pi
Write-Host "--> [4/4] Activating container on the Raspberry Pi..." -ForegroundColor Blue
ssh pi@192.168.0.10 "
...
Write-Host "Web UI: http://192.168.0.10:8080"
Write-Host "==========================================================" -ForegroundColor Green
